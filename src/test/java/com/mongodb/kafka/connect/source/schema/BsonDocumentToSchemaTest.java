...
public class BsonDocumentToSchemaTest {
  ...
  @Test
  void testBigSchema() throws IOException {
    String json =
        new String(
            Files.readAllBytes(
                Paths.get("/Users/jeff.yemin/git/mongo-kafka/sampleDocument_238aa.txt")));

    BsonDocument document = parse(json);
    Schema schema = inferDocumentSchema(document);
    System.out.println(schema);
  }

  private static void diffSchemas(
      final String indent, final Schema firstSchema, final Schema secondSchema) {
    if (firstSchema.type() != secondSchema.type()) {
      System.out.println(
          "First schema type "
              + firstSchema.type()
              + " is not equal to second schema type "
              + secondSchema.type());
    }
    if (firstSchema.type() == STRUCT) {
      diffStructTypeSchemas(indent, firstSchema, secondSchema);
    } else if (firstSchema.type() == ARRAY) {
      diffSchemas(indent + "  ", firstSchema.valueSchema(), secondSchema.valueSchema());
    }
  }

  private static void diffStructTypeSchemas(
      final String indent, final Schema firstSchema, final Schema secondSchema) {
